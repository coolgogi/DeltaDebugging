...
MSG=(jsondump.c:44 xmlDictComputeFastQKey xmlParseXMLDecl xmlNewDocElementContent __asan_memcpy exprAnalyze)

for (( i=0; i < ${#TARGET[@]}; i++)) do
	$RANGE ${TARGET[$i]} ${INPUT[$i]} ${MSG[$i]} >> compare
	wc -c temp >> compare
	$THREAD ${TARGET[$i]} ${INPUT[$i]} ${MSG[$i]} 2 >> compare
	wc -c temp >> compare
	$THREAD ${TARGET[$i]} ${INPUT[$i]} ${MSG[$i]} 4 >> compare
	wc -c temp >> compare
	$THREAD ${TARGET[$i]} ${INPUT[$i]} ${MSG[$i]} 6 >> compare
	wc -c temp >> compare
	$THREAD ${TARGET[$i]} ${INPUT[$i]} ${MSG[$i]} 8 >> compare
	wc -c temp >> compare
done
...
echo "libxml2/crash-d8960e2 no option" >> compare
#$RANGE $TARGET $INPUT $MSG $P &>> compare
wc -c temp >> compare
#$THREAD $TARGET $INPUT $MSG &>> compare
wc -c temp >> compare

echo "libxml2/crash-50b12d3 no option" >> compare
#$RANGE $TARGET $INPUT $MSG $P &>> compare
wc -c temp >> compare
#$THREAD $TARGET $INPUT $MSG &>> compare
wc -c temp >> compare

echo "libxml2/leak-bdbb285 no option" >> compare
#$RANGE $TARGET $INPUT $MSG $P &>> compare
wc -c temp >> compare
#$THREAD $TARGET $INPUT $MSG &>> compare
wc -c temp >> compare

echo "sqlite/crash-1066e42 no option" >> compare
#$RANGE $TARGET $INPUT $MSG $P &>> compare
wc -c temp >> compare
#$THREAD $TARGET $INPUT $MSG &>> compare
wc -c temp >> compare

echo "sqlite/crash-0adc497 no option" >> compare
#$RANGE $TARGET $INPUT $MSG $P &>> compare
wc -c temp >> compare
#$THREAD $TARGET $INPUT $MSG &>> compare
wc -c temp >> compare
